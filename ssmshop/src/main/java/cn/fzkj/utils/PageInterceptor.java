...
//		}
		if (obj instanceof PageBean<?>) {				//拦截参数为PageBean对象的，其他放过
			...
			// 给当前的pageBean参数对象设置总记录数
			this.setTotalRecord(pageBean, mappedStatement, connection);
			// 获取分页Sql语句
			String pageSql = this.getPageSql(pageBean, sql);
	 ...
	 * @return
	 */
	private String getPageSql(PageBean<?> pageBean, String sql) {
		StringBuffer sqlBuffer = new StringBuffer(sql);
		if ("mysql".equalsIgnoreCase(databaseType)) {
			return getMysqlPageSql(pageBean, sqlBuffer);
		} else if ("oracle".equalsIgnoreCase(databaseType)) {
			return getOraclePageSql(pageBean, sqlBuffer);
		}
	 ...
	 * @return Mysql数据库分页语句
	 */
	private String getMysqlPageSql(PageBean<?> pageBean, StringBuffer sqlBuffer) {
		// 计算第一条记录的位置，Mysql中记录的位置是从0开始的。
		int offset = (pageBean.getPage() - 1) * pageBean.getLimit();
		sqlBuffer.append(" limit ").append(offset).append(",").append(pageBean.getLimit());
		return sqlBuffer.toString();
	}
	 ...
	 */
	private String getOraclePageSql(PageBean<?> pageBean, StringBuffer sqlBuffer) {
		// 计算第一条记录的位置，Oracle分页是通过rownum进行的，而rownum是从1开始的
		int offset = (pageBean.getPage() - 1) * pageBean.getLimit() + 1;
		sqlBuffer.insert(0, "select u.*, rownum r from (").append(") u where rownum < ").append(offset + pageBean.getLimit());
	 ...
	 */
	private void setTotalRecord(PageBean<?> pageBean, MappedStatement mappedStatement,
		...
		BoundSql countBoundSql = new BoundSql(
				...
				parameterMappings, pageBean);
		// 通过mappedStatement、参数对象page和BoundSql对象countBoundSql建立一个用于设定参数的ParameterHandler对象
		ParameterHandler parameterHandler = new DefaultParameterHandler(
				mappedStatement, pageBean, countBoundSql);
